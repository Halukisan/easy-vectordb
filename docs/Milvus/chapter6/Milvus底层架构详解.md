...
> 💡 **版本说明**：本文以 Milvus 2.x 经典架构为主线进行讲解，并在各关键位置用 ⛳ 标注最新版本的架构演进内容。经典架构的描述在对应版本下完全正确，演进内容帮助你理解架构的发展脉络。
...
> ⛳ **架构演进提示**：以上四层架构（接入层→协调服务→工作节点→存储层）是 Milvus 的经典分层模型，在 2.x 早期版本中，协调服务由四个独立的协调器（Root Coord / Query Coord / Data Coord / Index Coord）组成，工作节点也只有 Query Node、Data Node 和 Index Node 三种。但随着 Milvus 在大规模生产场景中的深入应用，这种”四协调器”架构暴露出协调器间通信开销大、状态同步复杂、故障恢复链路长等问题。因此在最新版本中，Milvus 对架构进行了重要演进——**四协调器统一为一个 Coordinator，同时引入了全新的 Streaming Node**。下表先列出经典架构的组件，之后我们会在各组件详解中逐步介绍新架构的变化。
...
<details>
<summary>📦 最新版本架构组件对照表（点击展开）</summary>

| 组件 | 存储内容 | 后端技术 | 与经典架构的差异 |
| :--- | :--- | :--- | :--- |
| **WAL Storage** | 数据变更日志（Insert/Delete/Update） | Kafka/Pulsar/Woodpecker | 由 Log Broker 更名，新增 Woodpecker 后端 |
| **Object Storage** | 向量原始数据、索引文件、Segment 文件 | MinIO/S3/Azure Blob/GCS/本地文件 | 不变 |
| **Meta Storage** | 集合Schema、Segment状态、消费检查点、TSO | ETCD | 由 Metadata Storage 更名，去掉 MySQL/TiDB |
| **Streaming Node** 🆕 | *Growing数据查询*、查询计划生成、流批转换 | 本地Memory | **全新组件**，接管 Query Node 的实时数据处理 |
| **Query Node** | *Sealed数据查询*、内存索引、执行引擎 | 本地SSD/Memory/GPU | 职责收窄，只处理已Flush的Sealed数据 |
| **Data Node** | 历史数据离线处理（Compaction+索引构建） | 本地磁盘 | 合并了 Index Node 的索引构建职责 |
...
> ⛳ **架构演进提示**：上述"读写分离"和"日志即数据"的设计理念在经典架构中表现良好，但也存在一个痛点——**Query Node 既要处理已 Flush 的 Sealed 数据，又要处理还在内存中的 Growing 数据**，导致查询逻辑复杂且延迟不稳定。最新版本中，Milvus 将"读写分离"演进为**"流批分离"**：新增的 Streaming Node 专门处理流数据（Growing Segment），Query Node 只处理批数据（Sealed Segment），Data Node 负责离线处理（Compaction + 索引构建）。同时"日志即数据"的理念不变，但 Log Broker 更名为 **WAL Storage**，并引入了云原生零磁盘实现 Woodpecker，让"流批一体"的落地更加优雅。我们会在后续组件详解中展开这些变化。
...
6. **增量模型**：该模型是milvus中非常重要的一部分，你可以理解为Milvus采用"基础数据+增量变更"的存储模式，类似于数据库的WAL（Write-Ahead Log）机制。
   ...
   **核心思想**：
   ...
   - **实时合并**：Query Node在查询时动态合并基础数据+增量变更，呈现最新视图（*最新版本中，Growing数据的实时合并工作已由Streaming Node接管，Query Node只处理Sealed数据*）
```
# 时间轴
t0: 插入向量 [A]
t1: 插入向量 [B] 
...
> ⛳ **架构演进提示**：在最新版本中，Root Coordinator 和 Data Coordinator 已统一为一个 **Coordinator**，因此 TSO 的分配和维护不再跨协调器传递，而是由 Coordinator 内部统一完成。这简化了 TSO 的管理链路——以前是 Root Coord 分配 → 通知 Data Coord → Data Coord 分发给 Data Node，现在直接由 Coordinator 分配并分发给 Data Node，减少了跨进程通信的开销。但整体流程逻辑（分配TSO→写日志→Data Node消费→Flush→更新水印）不变，上面的伪代码仍然有效，只需将 `RootCoordinator()` 和 `DataCoordinator()` 视为 Coordinator 的内部模块即可。
```python
...
> ⛳ **架构演进提示**：在最新版本中，Data Coordinator 和 Data Node 的负载均衡逻辑统一由 **Coordinator** 管理，不再有独立的 Data Coord 进程。但负载均衡的本质不变——仍然是 Coordinator（应用层）管理 Segment 分配，k8s（基础设施层）管理 Pod 扩缩容，两者分工明确。新的 Pod 启动后仍然会自动向 Coordinator 注册，只是注册的目标从 Data Coord 变为了统一的 Coordinator。
...
> ⛳ **架构演进：从四协调器到统一Coordinator**
>
> 上面我们详细介绍了 Milvus 经典架构中的四个独立协调器——Root Coord、Query Coord、Data Coord、Index Coord。这种"分而治之"的设计在早期版本中职责清晰、易于理解，但在大规模生产环境中逐渐暴露出几个问题：
>
> 1. **协调器间通信开销**：Root Coord 分配 TSO 后要通知 Data Coord，Data Coord 处理完 Segment 后要通知 Index Coord 构建索引……这些跨进程通信增加了延迟和出错概率。
> 2. **状态同步复杂**：四个协调器的状态分别存储在 ETCD 中，容易出现短暂的不一致（比如 Data Coord 已经分配了 Segment，但 Query Coord 还不知道）。
> 3. **故障恢复链路长**：一个协调器故障后，需要等待它恢复或重新选举，期间它管理的功能全部不可用，影响面大。
>
> 因此，在最新版本中，Milvus 将四个协调器**统一为一个 Coordinator**，但内部仍然保留功能模块划分。你可以这样理解——从"四个独立部门"变成了"一个部门下的四个小组"：
>
> | 经典架构 | 新架构 | 变化说明 |
> | :--- | :--- | :--- |
> | Root Coordinator | Coordinator 的 **DDL/DCL 管理模块** | 功能不变，成为内部模块 |
> | Root Coordinator (TSO) | Coordinator 的 **TSO 分配模块** | 不再跨进程传递TSO |
> | Query Coordinator | Coordinator 的 **查询管理模块** | 功能不变，成为内部模块 |
> | Data Coordinator | Coordinator 的 **历史数据管理模块** | 合并了 Index Coord 的职责 |
> | Index Coordinator | ~~已移除~~ | 职责合并进历史数据管理模块 |
> | — | Coordinator 的 **Streaming 服务管理模块** 🆕 | 新增，管理 Streaming Node |
>
> 这样做的好处是：**对外统一入口（部署运维简单），对内仍然分工（功能不丢失）**。四个协调器之间的跨进程通信变成了进程内的模块调用，延迟更低、一致性更强、故障恢复更简单。
>
> 接下来我们介绍工作节点时，也会看到类似的演进——新增 Streaming Node，合并 Index Node。
...
> ⛳ **架构演进：新增 Streaming Node，合并 Index Node**
>
> 上面我们介绍了经典架构中的三种工作节点——Query Node、Data Node、Index Node。在最新版本中，工作节点的分工发生了两个重要变化：
>
> **变化一：新增 Streaming Node（流式节点）** 🆕
>
> 这是新架构中**最重要的新增组件**。为什么要引入它？回忆一下，在经典架构中，Query Node 既要处理已经 Flush 到 Object Storage 的 Sealed 数据，又要订阅 Log Broker 处理还在内存中的 Growing 数据——一个节点干了两种性质完全不同的活。Growing 数据是"流"（实时、低延迟），Sealed 数据是"批"（历史、高吞吐），两者的优化目标完全不同，混在一起导致查询逻辑复杂且延迟不稳定。
>
> Streaming Node 的出现就是为了解决这个问题。它专职处理**尚未 Flush 的 Growing Segment 数据**：
> - **Growing 数据查询**：提供刚写入数据的实时搜索能力，让"写入后立即可查"不再是空话
> - **查询计划生成**：为搜索请求生成查询计划，决定哪些子任务自己处理（Growing 数据），哪些分发给 Query Node（Sealed 数据）
> - **流批转换**：当 Growing Segment 达到容量阈值时，触发转换为 Sealed Segment，通知 Coordinator 更新元数据
>
> 你可以把它理解为每个分片的"迷你大脑"——它知道自己分片上的实时数据状态，能够做出本地的查询决策，不需要凡事都去问 Coordinator。
>
> **变化二：Index Node 合并进 Data Node**
>
> Index Node 的职责是为 Sealed Segment 构建索引，而 Data Node 的职责是 Compaction（合并小 Segment、清理删除数据）。这两个任务有一个共同点——它们都是**离线批处理任务**，而且索引构建的输入就是 Data Node Flush 出来的 Segment 文件。把它们放在同一个节点上，可以避免节点间不必要的数据传输，提高资源利用率。因此，最新版本中 Index Node 不再独立存在，其索引构建职责合并进了 Data Node。
>
> | 经典架构 | 新架构 | 变化说明 |
> | :--- | :--- | :--- |
> | Query Node（Sealed + Growing） | **Streaming Node**（Growing） + **Query Node**（Sealed） | 拆分，各司其职 |
> | Data Node（写入缓冲 + Compaction） | **Data Node**（Compaction + 索引构建） | 合并 Index Node 职责 |
> | Index Node（索引构建） | ~~已移除~~ | 合并进 Data Node |
...
> ⛳ **架构演进提示**：在最新版本中，存储层有三处变化：
>
> 1. **Metadata Storage → Meta Storage**：名称简化，并且去掉了 MySQL/TiDB 的支持，只保留 ETCD。原因是 Milvus 的元数据特点是强一致性要求+高频小事务，ETCD 的 Raft 协议天然适合，MySQL/TiDB 在这种场景下反而引入了不必要的运维复杂度。
>
> 2. **Log Broker → WAL Storage**：更名体现了定位变化——从"消息代理"变为"预写日志存储"。同时新增了 **Woodpecker** 后端，这是 Milvus 官方推出的云原生零磁盘 WAL 实现，直接写入对象存储（S3/MinIO），不需要像 Kafka/Pulsar 那样维护本地日志磁盘，对运维友好度是巨大提升。三种后端可以按需选择：Kafka（高性能）、Pulsar（功能丰富）、Woodpecker（零运维）。
>
> 3. **Root Coord 是 Metadata Storage 主要使用者**这一点不再适用——因为 Coordinator 统一管理后，所有模块共享同一个 Meta Storage 访问层，不再有"主要使用者"的概念。
...
> ⛳ **架构演进：最新版本的数据写入流程**
>
> 上面我们详细介绍了经典架构下的10步写入流程。在最新版本中，由于引入了 Streaming Node 和统一的 Coordinator，写入流程变得更简洁。核心变化是——**Streaming Node 成为写入的"第一站"**：
>
> 1.  **用户请求**：客户端通过 SDK (如 `pymilvus.insert()`) 发起插入请求。
> 2.  **接入代理**：请求到达 Proxy 网关。
> 3.  **验证与转发**：Proxy 验证请求合法性，负载均衡转发。
> 4.  **流式节点处理 (Streaming Node)** 🆕：请求被路由到对应分片的 Streaming Node。Streaming Node 为操作分配 TSO（不再需要跨协调器传递），并将操作日志写入 WAL Storage。
> 5.  **实时可查询**：写入 WAL 后，数据立即在 Streaming Node 上可被查询（Growing Segment）。这就是"流批一体"在写流程中的体现——写入后立即可查，不需要等待 Flush。
> 6.  **流批转换**：当 Growing Segment 达到容量阈值时，Streaming Node 触发转换为 Sealed Segment，通知 Coordinator 更新元数据。
> 7.  **数据节点处理 (Data Node)**：Data Node 从 WAL Storage 消费数据，执行 Compaction 和索引构建（原 Index Node 的职责），将 Sealed Segment 原始数据和索引文件写入 Object Storage。
> 8.  **查询节点加载**：Query Node 加载新构建的索引，替换对应的 Growing 数据。
>
> 对比经典架构的10步流程，你会注意到几个关键简化：
> - **步骤4-5合并**：不再需要"Root Coord 分配 TSO → 通知 Data Coord"这个跨协调器传递环节，Streaming Node 直接分配 TSO 并写入 WAL
> - **步骤6前置**：经典架构中，数据要先写 Log Broker → Data Node 消费 → 写内存缓冲 → 才能被 Query Node 查询；新架构中，写入 WAL 后立即可查，因为 Streaming Node 直接提供 Growing 数据查询
> - **步骤9-10合并**：Index Node 不再独立存在，索引构建由 Data Node 完成，省去了 Data Coord → Index Coord → Index Node 的调度链路
>
> 整体来看，新架构把写入路径从"Proxy → Root Coord → Data Coord → Log Broker → Data Node → Index Coord → Index Node"简化为"Proxy → Streaming Node → WAL Storage → Data Node"，链路更短、延迟更低、组件更少。
...
> ⛳ **架构演进：最新版本的查询流程**
>
> 上面我们详细介绍了经典架构下的7步查询流程。在最新版本中，由于 Streaming Node 的引入，查询流程的核心变化是——**Streaming Node 成为查询的"协调者"**，并且引入了**三级归并机制**：
>
> 1.  **用户请求**：客户端通过 SDK 发起搜索请求。
> 2.  **接入代理**：请求到达 Proxy 网关。
> 3.  **路由与分发**：Proxy 使用路由缓存确定目标节点，将搜索请求转发到对应的 **Streaming Node**（而不是像经典架构那样先转发给 Query Coord）。
> 4.  **流式节点协调 (Streaming Node)** 🆕：Streaming Node 生成查询计划，本地执行 Growing Segment 数据的搜索，同时将 Sealed Segment 的搜索子任务分发给 Query Node。
> 5.  **查询节点执行 (Query Node)**：与经典架构相同——检查缓存、冷数据加载、标量过滤→ANN搜索→向量距离计算→局部TopK。但只处理 Sealed 数据。
> 6.  **三级归并** 🆕：
>     - **第一级**：Query Node 内部归并多个 Segment 结果（和经典架构一样）
>     - **第二级**：Streaming Node 归并多个 Query Node 的局部 TopK + 自己处理的 Growing 数据结果（*新增*）
>     - **第三级**：Proxy 归并所有 Streaming Node 结果，得到全局 TopK
> 7.  **结果返回**：最终结果通过 Proxy 返回给客户端 SDK。
>
> 对比经典架构，最核心的差异在于：
> - **Query Coord 的协调角色被 Streaming Node 接管**：Streaming Node 更了解数据的实时状态（哪些是 Growing、哪些是 Sealed），可以做出更精准的查询计划
> - **二级归并→三级归并**：经典架构是"Query Node 局部归并 → Query Coord 全局归并"两级，新架构增加了"Streaming Node 归并 Growing + Sealed 结果"这一级，使得 Growing 数据的查询不再需要走 Query Node 的完整链路
> - **Growing 数据查询延迟大幅降低**：经典架构中，Growing 数据需要被 Query Node 订阅消费后才能查询；新架构中，Streaming Node 直接提供 Growing 数据查询，省去了中间环节
...
> ⛳ **架构演进补充**：在最新版本中，上述架构优势有了进一步的强化：
> - **读写分离 → 流批分离**：Streaming Node 处理流数据（Growing Segment），Query Node 处理批数据（Sealed Segment），Data Node 负责离线处理（Compaction + 索引构建），三者职责更清晰，优化目标更明确——Streaming Node 优化写入延迟，Query Node 优化查询延迟，Data Node 优化批处理吞吐。
> - **流批一体的落地更优雅**：经典架构中，Query Node 通过订阅 Log Broker 实现"流批一体"，但一个节点同时处理两种数据导致逻辑复杂；新架构中，Streaming Node 和 Query Node 各自专注一种数据，通过协作实现流批统一视图，架构更清晰。
> - **索引异步构建不变，但链路更短**：不再需要 Data Coord → Index Coord → Index Node 的三级调度，直接由 Coordinator → Data Node 完成索引构建。
> - **性能关注点新增**：Streaming Node 的内存是否足够缓存 Growing 数据？WAL Storage（特别是 Woodpecker）的写入延迟是否满足实时查询要求？
...
> ⛳ **架构演进补充**：如果用最新版本的架构来重新做上面的总结，会是这样：
>
> **协调器（Coordinator）**：统一管理集群的元数据、任务调度和一致性保证，不再有四个独立的协调器。内部按功能模块划分：DDL/DCL管理模块（原Root Coord）、TSO分配模块（原Root Coord的TSO职责）、Streaming服务管理模块（🆕，管理Streaming Node）、查询管理模块（原Query Coord）、历史数据管理模块（原Data Coord + Index Coord合并）。
>
> **Node组件**：从三种变为三种（但内容不同）——Streaming Node（🆕，处理Growing数据的实时查询和流批转换）、Query Node（只处理Sealed数据）、Data Node（合并了Index Node的索引构建职责）。
>
> **Storage组件**：WAL Storage（原Log Broker，新增Woodpecker后端）、Meta Storage（原Metadata Storage，只保留ETCD）、Object Storage（不变）。
>
> 你会发现，虽然组件名称和数量变了，但**核心职责的划分逻辑没有变**——仍然是"协调层负责调度、工作层负责执行、存储层负责持久化"，只是每一层内部的分工更加精细和合理了。
...
> ⛳ **架构演进提示**：上面我们讨论的"查询协调器和数据协调器通过etcd共享segment状态"的机制，在最新版本中有了更优雅的实现——由于 Coordinator 是统一进程，查询管理和数据管理之间的状态共享不再需要通过 etcd 中转，而是直接在进程内完成。同时，"查询节点和数据节点无需直接通信"这一点仍然成立，但加入了 Streaming Node 作为中间层：**Streaming Node 消费 WAL Storage 的实时数据，Data Node 消费 WAL Storage 的历史数据**，两者仍然不需要直接通信，但通过 Streaming Node 的协调，查询层能更快地获取到最新数据。这种发布-订阅的核心思想没有变，只是实现更加简洁了。
